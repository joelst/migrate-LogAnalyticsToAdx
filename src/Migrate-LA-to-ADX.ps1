...
PARAM(    
    ...
    [Parameter(Mandatory = $true, HelpMessage="Enter the Azure Data Explorer (ADX) cluster name.")] 
    [string]$AdxClusterURL,

    [Parameter(Mandatory = $true, HelpMessage="Enter the Azure Data Explorer (ADX) cluster database name.")]
    [string]$AdxDBName,

    [Parameter(Mandatory = $false, HelpMessage="Enter the Azure Data Explorer (ADX) Engine URL.")]
    [string]$AdxEngineUrl = "$AdxClusterURL/$AdxDBName",

    [Parameter(Mandatory = $false, HelpMessage="Enter the host from which to download the Kusto Tools.")]
    [string]$KustoToolsPackage = "microsoft.azure.kusto.tools",

    [Parameter(Mandatory = $false, HelpMessage="Enter the Kusto Connection string.")]
    [string]$KustoConnectionString = "$AdxEngineUrl;Fed=True",
  
    [Parameter(Mandatory = $false, HelpMessage="Enter the location of the NuGet packages on the local computer.")]
    [string]$NuGetPackageLocation = "$($env:USERPROFILE)\.nuget\packages",

    [Parameter(Mandatory = $false, HelpMessage="Enter the NuGet index URL.")]
    [string]$NuGetIndex = "https://api.nuget.org/v3/index.json",
    
    [Parameter(Mandatory = $false, HelpMessage="Enter the NuGet download URL")]
...
Function CheckModules($module) {
    try {
        ...
        Write-Error "An error occurred in CheckModules() method" -ErrorAction Stop       
        exit
    }
}
...
Function Split-ArrayBySize() {
    ...
    try {
        Write-Host "`nSplitting array into groups of up to $ArraySize"
    ...
    try {
        ...
        foreach ($adxEventHub in $adxEventHubs) {        
            ...
            if ($adxEventHubs.Count -gt 1) {
                Write-Debug "`$adxEventhubs.count -gt 1"
                $exportRuleTables = '"{0}"' -f ($tablesArrayCollection[$count] -join '","')
            }
            else {
                Write-Debug "`$adxEventhubs.count -eq 1"
                $exportRuleTables = '"{0}"' -f ($tablesArrayCollection -join '","')
            }

            Write-Debug "`$eventHubNameSpace.ProvisioningState -eq $($eventHubNameSpace.ProvisioningState)"
...
Function New-ADXDataConnection() {
    ...
    try {   
        ...
        if("" -eq $AdxClusterName)
        {
            ...
            $AdxClusterName = $AdxClusterUrl.split('.')[0]
